...
export async function addCredits(req: AuthRequest, res: Response) {
  try {
    const db = getDb();
...
export async function getStats(req: AuthRequest, res: Response) {
  try {
    ...
    const [totalSupporters, totalCreators, totalAdmins, creditSum, totalPayments] =
      await Promise.all([
...
export async function grantRegistrationCredits(email: string, role: string) {
  try {
    const db = getDb();
