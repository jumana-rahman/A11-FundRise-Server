...
export async function create(req, res) {
    try {
        const db = getDb();
        const { campaignId, campaignTitle, contributionAmount, creatorEmail, creatorName, } = req.body;
        ...
        await db.collection("user").updateOne({ email: req.user.email }, { $inc: { credits: -amount } });
        const contribution = {
            campaignId,
...
export async function listBySupporter(req, res) {
    try {
        const db = getDb();
...
export async function listPendingForCreator(req, res) {
    try {
        const db = getDb();
...
export async function listApprovedForSupporter(req, res) {
    try {
        const db = getDb();
        const contributions = await db
            .collection(COLLECTION)
...
export async function approve(req, res) {
    try {
        ...
        await db.collection(COLLECTION).updateOne({ _id: new ObjectId(id) }, { $set: { status: "approved" } });
        // Add to campaign's raised amount
        await db.collection("campaigns").updateOne({ _id: new ObjectId(contribution.campaignId) }, { $inc: { amountRaised: contribution.contributionAmount } });
        // Notify supporter
        await db.collection("notifications").insertOne({
...
export async function reject(req, res) {
    try {
        ...
        // Update contribution status
        await db.collection(COLLECTION).updateOne({ _id: new ObjectId(id) }, { $set: { status: "rejected" } });
        // Refund credits to supporter
        await db.collection("user").updateOne({ email: contribution.supporterEmail }, { $inc: { credits: contribution.contributionAmount } });
        // Notify supporter
        await db.collection("notifications").insertOne({
...
export async function listByCampaign(req, res) {
    try {
        const db = getDb();
