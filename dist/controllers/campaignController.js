...
export async function getTopFunded(req, res) {
    try {
        const db = getDb();
        const campaigns = await db
            .collection(COLLECTION)
...
export async function create(req, res) {
    try {
        const db = getDb();
        const { campaignTitle, campaignStory, category, fundingGoal, minimumContribution, deadline, rewardInfo, campaignImageUrl, } = req.body;
...
export async function update(req, res) {
    try {
        ...
        if (campaignTitle !== undefined)
            updateData.campaignTitle = campaignTitle;
        if (campaignStory !== undefined)
            updateData.campaignStory = campaignStory;
        if (rewardInfo !== undefined)
            updateData.rewardInfo = rewardInfo;
...
export async function remove(req, res) {
    try {
        ...
        for (const contrib of approvedContributions) {
            await db
                .collection("user")
                .updateOne({ email: contrib.supporterEmail }, { $inc: { credits: contrib.contributionAmount } });
            await db
                .collection("contributions")
