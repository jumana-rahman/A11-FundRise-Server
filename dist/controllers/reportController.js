...
export async function resolve(req, res) {
    try {
        ...
        await db.collection(COLLECTION).updateOne({ _id: new ObjectId(id) }, { $set: { status: "resolved" } });
        ...
        if (action === "delete" && ObjectId.isValid(report.campaignId)) {
            ...
            await db.collection("campaigns").updateOne({ _id: new ObjectId(report.campaignId) }, { $set: { status: "rejected" } });
        }
